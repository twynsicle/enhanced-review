...
/**
 * The Agent SDK message loop. It knows how to read a stream of SDK messages
 * and nothing else: no configuration, no policy about what a failure means.
 * The caller supplies the options and decides what to do with the outcome —
 * `er` turns a failure into a message and a file on disk it can resume from.
 *
