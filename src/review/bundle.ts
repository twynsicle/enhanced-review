 ...
 * `er` writes one into each `review.html`.
 *
...
/** One side of a file as the reader's diff draws it. */
export type FileSide =
  | { kind: 'content'; content: string; language: string; lineCount: number }
  | { kind: 'absent' }
  | { kind: 'too-large' };
...
function toSide(side: EmbeddedSide | undefined, path: string): FileSide {
  if (!side || side.kind === 'absent') return { kind: 'absent' };
  if (side.kind === 'too-large') return side;
  const { content } = side;
  return {
    kind: 'content',
    content,
