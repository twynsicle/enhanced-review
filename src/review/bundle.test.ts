...
describe('filePair', () => {
  it('returns both sides with a detected language and line counts', () => {
    ...
    expect(pair.head).toMatchObject({ kind: 'content', lineCount: 3 });
  });

  it('reads an absent side as absent, which the reader shows as an added file', () => {
    expect(filePair(bundle, 'src/added.ts').base).toEqual({ kind: 'absent' });
  });

  it('passes too-large through', () => {
    expect(filePair(bundle, 'dist/huge.js').head).toEqual({ kind: 'too-large' });
  });

  it('reads a path the bundle does not hold as absent on both sides', () => {
    const pair = filePair(bundle, 'nowhere.ts');
    expect(pair.base).toEqual({ kind: 'absent' });
    expect(pair.head).toEqual({ kind: 'absent' });
  });
});
