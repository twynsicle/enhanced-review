...
describe('the finding table', () => {
  ...
  it('parses back from storage, and refuses a code it does not know', () => {
    const stored: unknown = JSON.parse(JSON.stringify(finding('diagram-dropped', 'cut short')));
    expect(FindingSchema.parse(stored)).toEqual(finding('diagram-dropped', 'cut short'));
    expect(
...
describe('a log of findings', () => {
  ...
  it('separates the disqualifying findings from the rest', () => {
    const findings = [
      ...
      finding('diagram-dropped', 'a warning'),
      finding('chapter-no-hunks', 'another hole'),
    ];
    expect(fatalFindings(findings).map((f) => f.message)).toEqual(['a hole', 'another hole']);
    expect(fatalFindings([finding('diagram-dropped', 'a warning')])).toEqual([]);
  });
});

describe('what a run earns by how it behaved', () => {
  it('says how it ended, and what to raise', () => {
    expect(runStoppedEarly('error_max_turns')).toEqual(
      finding(
        'run-stopped-early',
        'The run did not finish cleanly (error_max_turns), so the reviewer stopped short of the change. Raise --max-turns or --timeout and run again from the run stage.',
      ),
    );
  });
