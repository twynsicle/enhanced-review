 ...
 * `er-sidebar`, and applied as `--review-sidebar-width` on `<html>` rather than
 * as an inline style on the grid, so a drag can paint the variable directly
 * without re-rendering the reader (`applySidebarWidth`).
 *
...
export function followSidebarWidth(): () => void {
