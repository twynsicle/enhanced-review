...
/**
 * Rehydrate from storage. Called from the toggle's mount effect, as its
 * neighbours are. The editors subscribe to this store rather than being handed
 * the stored value, so one arriving after a diff has mounted is picked up as an
 * ordinary reflow — which is why nothing here needs a pre-paint script.
 */
export function bindDiffWrap(): void {
