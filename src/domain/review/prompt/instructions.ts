...
- Each chapter may have 0–3 insights — they are optional, not a quota. Add one only when it tells the reviewer something the chapter description does not already say. Insight types are reviewer aids (not AI opinions):
  - "context": outside knowledge the reviewer would otherwise have to look up themselves — how a library, API, or pattern actually behaves, with a link when one is genuinely useful
  ...
  - "reference": pointers to related code, docs, or an external resource
