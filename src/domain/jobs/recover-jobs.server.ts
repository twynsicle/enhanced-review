...
/**
 * Orphan recovery. Jobs run in-process, so any row still `pending` or
 * `running` when a process starts belonged to a process that is gone; they
 * can never finish. Flip them to `error` so the UI stops polling and the user
 * can rerun. Runs at server boot (`bootJobs`) and as the `recover-jobs`
 * one-shot.
 */
