...
**Status:** executing — Phases 0–5 complete, Phase 6 next. **Branch:** `migrate-react-router`, cut from `main` (`d63b87c`).
...
| D8  | Runner + one-shot jobs   | Review runner stays fire-and-forget **in the Express process** (AbortController registry, drain-before-done unchanged). `src/jobs/` gets `recover-jobs` (flip orphan `running` rows after a restart — new; PB had no equivalent); the container entrypoint runs migrate → recover → web. **Amended in Phase 5:** the Node-only Vite bundle is dropped (the CLI runs natively under type stripping, P5-D1) and `seed-allowlist` is gone with the allowlist feature (P5-D3). |
| D9  | Domain layout            | `src/domain/` split by concern: `auth/` (session/token helpers; the allowlist rule lived here until P5-D3 removed it), `github/` (Octokit wrapper + fetch helpers — absorbs `packages/github-client`), `review/` (runner, clone, executor, prompt, narrative types — absorbs `packages/review-types`), `jobs/` (lifecycle: create/cancel/rerun/timeout, registry, shutdown, recovery). `packages/` is removed.                                                             |
...
| **5** | **Container + jobs.** Runtime image carries the source the native entry points import, `prisma` CLI as a prod dependency, `entrypoint.sh` runs migrate → recover → web, image slimming, compose full flow, CI build + boot smoke. The allowlist feature is removed here (P5-D3).                                                                                              | Image builds **and boots** to a healthy `/api/health`. `docker compose up --build` from a clean clone works. `docker run <image> node src/jobs/cli.ts recover-jobs` works. No allowlist code or table remains. |
