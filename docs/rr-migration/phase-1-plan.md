...
| File                              | Rule (A4)                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| --------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `layering.guard.test.ts`          | `web` → `domain`, `db`, `common`, `config`; `domain` → `db`, `common`, `config`; `db` → `common`, `config`; `jobs` → `domain`, `db`, `common`, `config`; `common` → `config`; `config` imports nothing from `src/`. Inside `web`, only route modules, `root.tsx`, `entry.server.tsx` and `*.server.ts` may import `domain`/`db`/`config`/the logger (everything else is client-bundled). Nothing outside `src/web` and `server/` imports React, React Router or Mantine. |
...
- Guardrail sources live under `src/guardrails/` and are excluded from the
  rules they police (they quote the banned patterns). Assertions are
  `expect(report(violations)).toBe('')` so the failure lists every offender
  (oxlint's `vitest/valid-expect` rejects `expect(x, message)`).
