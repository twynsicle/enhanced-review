...
**Status:** decisions locked 2026-09-11. Phases 1–4 done; Phase 5 (docs and
the tidy-up) next. Phase 4's report leaves three checks that each need a real
paid run — see its Result.
...
| A5  | The report's entry point lives at `src/web/viewer/`, since only `src/web` may import React. It has its own `vite.viewer.config.ts` and builds everything inlined to `build/viewer/viewer.html`. The reader is wrapped in a hash **data** router (`createHashRouter`, because the diff always mounts a fetcher; see Phase 1), so `useSearchParams` and `?ch=` deep links work from `file://`. The render stage copies the built shell and injects the bundle JSON into a `<script type="application/json">` placeholder. It rebuilds the shell when it is missing, or when the source hash stamped in it is stale (after a `git pull`).                                                                                                                        |
