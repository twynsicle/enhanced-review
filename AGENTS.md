...
The backlog and all in-flight work live in Linear, in the team
...
- **Backlog items go to Linear, not to the repo.** When out-of-scope work
  turns up (a follow-up, a bug noticed in passing, something the user says to
  do "later"), search `ER` for an existing issue first and comment on it if
  one covers the work. Otherwise create one in `Backlog` with one label and a
  description that stands on its own: what was seen, where (file paths,
  ...
  the user's call.
- **Starting a task: find its issue first.** Any work that will end in a
  commit gets an issue. Questions, investigations that change nothing, and
  follow-ups inside a task that is already tracked do not. If the user names
  an issue, read it and its comments before planning. If they do not, search
  `ER` by keyword across every status, since the issue may already be sitting
  in `Backlog`. If nothing matches, create one. Then assign it to the user
  (`me`) and move it to `In Progress`.
...
- **Screenshots for anything visible.** When a change shows up in the UI,
  ...
  theming changed). Capture them to a file,
  because the upload needs one: the Playwright MCP's `browser_take_screenshot`
  writes a file, while the Browser pane's screenshot only returns an image to
  the conversation. Save them under `screenshots/`, which is gitignored. To
  upload a file, call `prepare_attachment_upload` with the file's exact byte
  size, `curl -X PUT --data-binary @file` to the returned URL sending every
  returned header verbatim within 60 s, then call
  `create_attachment_from_upload`. Upload one file at a time. Embed the
  `assetUrl` in the checkpoint comment as `![caption](assetUrl)` so the image
  shows inline.
