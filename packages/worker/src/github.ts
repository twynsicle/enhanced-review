/**
 * Minimal GitHub REST helpers needed by the worker. Kept as a tiny
 * `fetch` wrapper rather than depending on Octokit so the worker container
 * stays lean. We only need PR body + author for prompt-building; the
 * file-list comes from `git diff --numstat` locally.
 */
